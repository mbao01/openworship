...
test.describe("Display Page", () => {
  ...
  test("shows idle state when no content is pushed", async ({ page }) => {
    ...
    await expect(page.locator('[data-qa="display-idle"]')).toBeAttached();
  });

  test("shows watermark", async ({ page }) => {
    const watermark = page.locator('[data-qa="display-watermark"]');
  ...
  test("no content block visible in idle state", async ({ page }) => {
    ...
    await expect(page.locator('[data-qa="display-content"]')).not.toBeVisible();
  });
});
