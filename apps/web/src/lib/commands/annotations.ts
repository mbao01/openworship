...
/**
 * Pushes an ad-hoc custom slide (title + optional image) to the display.
 */
export async function pushCustomSlide(title: string, imageUrl?: string): Promise<void> {
  return invoke("push_custom_slide", { title, imageUrl });
}
