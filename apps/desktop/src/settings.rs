...
///
/// Persisted to `~/.openworship/tutorial.json`.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum TutorialState {
    /// Tour has not been started (fresh install or "Set up later" chosen).
    #[default]
    NotStarted,
    /// Tour is in progress, paused on step 1.
    InProgressStep1,
    /// Tour is in progress, paused on step 2.
    InProgressStep2,
    /// Tour is in progress, paused on step 3.
    InProgressStep3,
    /// Tour is in progress, paused on step 4.
    InProgressStep4,
    /// Tour is in progress, paused on step 5.
    InProgressStep5,
    /// Tour was completed successfully.
    Completed,
    /// User explicitly dismissed the tour.
    Dismissed,
}

/// Container for tutorial persistence.
...
impl TutorialSettings {
    /// Load from `~/.openworship/tutorial.json`, returning defaults on any error.
    pub fn load() -> Self {
    ...
    /// Persist to `~/.openworship/tutorial.json`.
    pub fn save(&self) -> Result<()> {
        let path = tutorial_path()?;
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent)?;
        }
        let json = serde_json::to_vec_pretty(self)?;
...
fn tutorial_path() -> Result<PathBuf> {
    let home = std::env::var("HOME").unwrap_or_else(|_| ".".into());
    Ok(PathBuf::from(home).join(".openworship").join("tutorial.json"))
}
